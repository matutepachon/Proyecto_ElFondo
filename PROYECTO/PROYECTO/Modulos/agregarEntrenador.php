    ...
    if ($input_correo && $input_contrasena) {
        ...
        if ($stmt->num_rows > 0) {
            ...
            if ($stmt->execute()) {
                ...
                $stmt = $conn->prepare("INSERT INTO Entrenador (ID_Usuario) VALUES (?)");
                $stmt->bind_param("i", $id_usuario);

                if ($stmt->execute()) {
                    $response = ['success' => true, 'mensaje' => 'Entrenador registrado con éxito.'];
                } else {
                    $response = ['success' => false, 'mensaje' => 'Error al registrar al enrenador: ' . $stmt->error];
                }
            } else {
